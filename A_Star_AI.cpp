...
{ 
    ...
    {   
        ...
        set<pair<ll,char>> pq;
        map<char,ll> visited;
        pq.insert({node_cost[start]+h_value[start],start});
        while(pq.size()){
            char parent=pq.begin()->se;
            if(parent==goal) break;
            ll valu=node_cost[pq.begin()->se];
            visited[parent]=1;
            pq.erase(pq.begin());
            for(auto child: connected[parent]){
                ...
                if(x>y) swap(x,y);
                ll valu_now=valu+edge_edge_cost[{x,y}]+h_value[child];
                if(valu_now<f_value[child]){
                    pq.erase({f_value[child],child});
                    node_cost[child]=valu+edge_edge_cost[{x,y}];
                    pq.insert({valu_now,child});
                    f_value[child]=valu_now;
                }
            }
        }
        cout << f_value[goal] nl;
    }
}
